...
In the end, we will use this to overwrite a variable at 0x5592a0412020.
Its current value is: This is a string that we want to overwrite.

Let's allocate the first chunk, taking space from the wilderness.
The chunk of 256 bytes has been allocated at 0x5592a190a1e0.
...
The top chunk starts at 0x5592a190a360
...
The value we want to write to at 0x5592a0412020, and the top chunk is at 0x5592a190a360, so accounting for the header size,
we will malloc 0xfffffffffeb07ca0 bytes.
As expected, the new pointer is at the same place as the old top chunk: 0x5592a190a360

Now, the next chunk we overwrite will point at our target buffer.
malloc(100) => 0x5592a0412020!
